...
import { 
  ...
  FileText,
  Edit2,
  Trash2 // Added for the Export button
} from 'lucide-react';
 ...
 const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchIncomeData = async () => {
      ...
      try {
        setIsLoading(true);
        // The backend expects range="monthly", "daily", etc.
        const rangeParam = activeTab.toLowerCase();
        const token=localStorage.getItem('token');
    const headers=token?{
      Authorization:`Bearer ${token}`
    }:{};
        // Update URL to match your Express routes
 
        const response = await axios.get(`${backendUrl}/api/income/overview?range=${rangeParam}`, {
    ...
    // ================= DAILY =================
      ...
      for (let i = 0; i < 24; i++) {
        const label = new Date(0, 0, 0, i).toLocaleString("default", {
          hour: "numeric",
        });
        hours.push(label);
        grouped[label] = { income: 0,  };
      }
  ...
  const getChartTitle = () => {
    switch (activeTab) {
      case 'Yearly': return 'Monthly Income Trends';
      case 'Monthly': return 'Daily Income Trends';
      case 'Weekly': return 'Daily Income Trends';
      case 'Daily': return 'Hourly Income Trends';
      default: return 'Income Trends';
    }
  };
  ...
  const getSubLabel = () => {
   ...
   const exporting=async()=>{
    ...
    const res=await axios.get(`${backendUrl}/api/income/export?range=${rangeParam}`,{
        headers:headers,
        responseType: 'blob'
    },
...
const url = window.URL.createObjectURL(new Blob([res.data]));
    ...
    a.href = url;
    a.download = "incomes.xlsx"; // file name
  ...
  const removeTransaction=async(id)=>{
    try{
      outletContext.deleteTranscation(id,"income");
  ...
  return (
    <div className="w-full min-h-screen bg-[#f8fafc] p-4 md:p-8 font-sans rounded-3xl">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-8">
            <div className="space-y-1.5 pt-1">
              <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">Income Overview</h1>
              <p className="text-gray-400 font-medium text-sm pt-1">Track and manage your income sources</p>
            </div>

            <div className="flex flex-col sm:items-end gap-5">
              <button onClick={() => {setType("income"),setOpen(true)}} className="group flex flex-none items-center justify-center space-x-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-xl font-semibold transition-all duration-300 shadow-[0_4px_14px_0_rgba(5,150,105,0.39)] hover:shadow-[0_6px_20px_rgba(5,150,105,0.23)] hover:-translate-y-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100 hover:border-emerald-100 transition-colors">
            <div className="flex items-center space-x-3 mb-4">
              <div className="p-2 bg-emerald-50 rounded-xl text-emerald-600">
                <DollarSign className="w-5 h-5" />
              </div>
              <span className="text-gray-500 font-semibold text-sm">Total Income</span>
            </div>
            <div className="text-3xl font-extrabold text-gray-900 mb-2">{dashboardData.totalIncome.toLocaleString("en-IN",{style:"currency",currency:"INR"})}</div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100 hover:border-blue-100 transition-colors">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <span className="text-gray-500 font-semibold text-sm">Average Income</span>
            </div>
            <div className="text-3xl font-extrabold text-gray-900 mb-2">{dashboardData.averageIncome.toLocaleString("en-IN",{style:"currency",currency:"INR"})}</div>
            <div className="flex items-center text-xs text-gray-400 font-medium space-x-1.5">
              <Calendar className="w-3.5 h-3.5" />
        ...
        <div className="bg-white rounded-3xl p-8 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            ...
            {/* Added Export Data button over chart area matching the design */}
            <button className="flex items-center space-x-1.5 px-4 py-2 border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors text-sm font-semibold max-w-max">
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis 
                  ...
                  dy={10}
                  interval={0}
                  minTickGap={100} // Ensures labels don't bunch up if there are many (like 30 days)
                />
                <YAxis 
                  axisLine={false} 
                  tickLine={false} 
                  tick={{ fill: '#047857', fontSize: 13, fontWeight: 500 }}
      ...
      <Add outletContext={outletContext} setOpen={setOpen} open={open} type={type} id={id} ></Add>
        <div className="bg-white rounded-3xl p-8 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100 mt-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-xl font-bold text-gray-900">Income Transactions</h2>
              <span className="text-sm font-medium text-gray-400">({getSubLabel()})</span>
            </div>
            ...
            {transactionsList.map((tx) => (
              <div key={tx._id} className="flex items-center justify-between py-4 border-b border-gray-50 last:border-b-0 hover:bg-gray-50/50 transition-colors rounded-2xl px-3 -mx-3 group">
                ...
                <div className="flex items-center space-x-4">
                  <div className="p-3 bg-emerald-50 rounded-xl text-emerald-600 group-hover:bg-green-100 transition">
                     {/* Dynamic icon check if you wanted to expand it */}
                    {tx.category === 'Salary' ? <TrendingUp className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                  </div>
                  <div>
                    <h3 className="text-gray-900 font-bold text-[15px] tracking-tight">{tx.description || 'Income'}</h3>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="flex items-center space-x-2 text-emerald-500/70 opacity-0 group-hover:opacity-100 transition-all duration-200">
                    <button onClick={()=>{
                      setType('editIncome');
                       setOpen(true);
                       setId(tx._id);
                    }} className="hover:text-emerald-700 hover:bg-emerald-50 p-1.5 rounded-lg transition-colors">
